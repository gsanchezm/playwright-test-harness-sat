...
|---|---|---|---|
| `/` (login) | Sign in with username/password or "Quick Login" persona buttons | textbox placeholder `standard_user`, textbox placeholder `••••••••`, button "Show password", button "Sign In", quick-login buttons `data-testid="user-<persona>"` (visible label is a short name like "Standard"/"Locked"; `title` attribute carries the real persona but is NOT the accessible name once visible text exists — use the testid, not `getByRole(name:)`), country flag buttons "Select US/MX/CH/JP/SA" | Username/Password `<label>` elements have no `for`/`id`/`aria-*` link to their inputs — `getByLabel` never resolves; use `getByPlaceholder` instead (confirmed via CI failure + live DOM inspection). Sign-in for `standard_user` took ~8s in manual testing (cold start or simulated latency) before redirecting to `/catalog` |
